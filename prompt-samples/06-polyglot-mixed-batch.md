<!--
  Scenario: 06-polyglot-mixed-batch
  ...
  Detected tags : ["nextjs","react","django","gin","node"]
  Batch files   : ["apps/web/app/actions.ts","services/api/views.py","services/edge/main.go"]
  Batch langs   : ["go","python","typescript"]
  Batch slugs   : ["all-server-actions","py-django-view","sql-injection","go-gin-route"]
...
- `go-gin-route`: Weak entry-point candidate — confirm no auth `r.Use(...)` precedes the route registration in this group.

## Target Files

- **apps/web/app/actions.ts**
    - [all-server-actions] L7: Server Action export
- **services/api/views.py**
    - [py-django-view] L11: class-based view
    - [sql-injection] L29: f-string SQL
- **services/edge/main.go**
    - [go-gin-route] L21: Gin method registration
