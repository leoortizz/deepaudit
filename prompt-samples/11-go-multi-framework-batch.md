<!--
  Scenario: 11-go-multi-framework-batch
  Go project that pulls in two Go frameworks (Gin + Chi) — both highlights ship for a Go-only batch.
  Detected tags : ["gin","chi","go"]
  Batch files   : ["cmd/server/main.go"]
  Batch langs   : ["go"]
  Batch slugs   : ["go-gin-route","go-chi-route","go-ssrf"]
...
- `go-chi-route`: Weak entry-point candidate — confirm `r.Use(auth)` is in scope and no `r.Mount(...)` later short-circuits the inheritance.
- `go-ssrf`: Concatenated URL passed to `http.Get` / `client.Do` without an allowlist — internal hosts are the high-severity case.
