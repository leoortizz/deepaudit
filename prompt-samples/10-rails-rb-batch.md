<!--
  ...
  Batch langs   : ["ruby"]
  Batch slugs   : ["rb-rails-controller","sql-injection","open-redirect"]
...
- `rb-rails-controller`: Weak entry-point candidate — confirm `skip_before_action :authenticate_user!` is intentional or that no auth callback is in scope.
