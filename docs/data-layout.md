...
`data/` is deepaudit's on-disk state. Each project owns a subdirectory; the
...
### `Triage` (set by `deepaudit triage`)
...
### `Revalidation` (set by `deepaudit revalidate`)
...
### `gitInfo` (set by `deepaudit enrich`)
...
reporting (`deepaudit status`) and for filtering exports by run.
...
Generated by `deepaudit report`. One markdown per project plus a JSON
...
For richer queries, prefer `deepaudit export --format json` — it applies
