...
import { DATA_EXCLUDES, DEEPAUDIT_APP_EXCLUDES, makeTarball, TARGET_EXCLUDES } from "./upload.js";

/** Commands where we inject --root to point at the sandbox target checkout */
const NEEDS_ROOT = new Set(["process", "revalidate", "scan"]);

/**
 * Build the (cmd, args) pair to run a deepaudit subcommand inside the sandbox.
 * Differs by mode: dev runs the source via tsx; installed runs the bin shim
 * left behind by `pnpm install` in the uploaded `.deepaudit/` workspace.
 */
function buildSandboxInvocation(
  ...
  if (mode === "installed") {
    return { cmd: "node_modules/.bin/deepaudit", args: tail };
  }
  return { cmd: "npx", args: ["tsx", "packages/deepaudit/src/cli.ts", ...tail] };
}
...
/**
 * Decide what directory to tarball as the "deepaudit app" for sandbox upload,
 * and which CLI invocation pattern to use inside the sandbox. Two modes:
 *
 *   - **dev**: this CLI is running from the source repo (the deepaudit package
 *     is found OUTSIDE any `node_modules/`). Tarball the source workspace
 *     root (the dir with `pnpm-workspace.yaml`) so the full monorepo +
 *     lockfile ship over. Workers run `tsx packages/deepaudit/src/cli.ts`.
 *
 *   - **installed**: this CLI is running from `node_modules/deepaudit/` inside
 *     a user's `.deepaudit/` workspace. Tarball that workspace dir (parent of
 *     `deepaudit.config.ts`) — it's a one-package npm project; the sandbox
 *     re-installs `deepaudit` from npm. Workers run `node_modules/.bin/deepaudit`.
 *
 * The deepaudit package is identified by `package.json:name === "deepaudit"`,
 * NOT by `pnpm-workspace.yaml`, so we don't accidentally pick the user's
 * parent monorepo if they happen to have one above `.deepaudit/`.
 */
function resolveDeepauditAppContext(): { root: string; mode: DeepauditMode } {
      ...
      try {
        ...
        if (pkg.name === "deepaudit") {
          if (dir.includes(`${path.sep}node_modules${path.sep}`)) {
            // Installed: ship the user's .deepaudit/ workspace, not the
            // node_modules/deepaudit package itself. The workspace has its
            // own package.json that depends on `deepaudit`, so the sandbox
            // re-resolves it (no fragile node_modules tarball).
            const cfg = getConfigPath();
            if (!cfg) {
              throw new Error(
                "deepaudit sandbox: no deepaudit.config.ts found above cwd. Sandbox needs a deepaudit workspace (run `deepaudit init` first).",
              );
            }
  ...
  throw new Error(
    "Could not locate the deepaudit package directory (no package.json with name 'deepaudit' found in any ancestor)",
  );
}
...
async function prepareUploads(
  ...
  const [app, target, data] = await Promise.all([
    makeTarball(appRoot, DEEPAUDIT_APP_EXCLUDES, onLog),
...
async function bootstrapAndSpawn(
  ...
  const ctx = resolveDeepauditAppContext();
...
async function dispatchOnSandbox(
  ...
  onLog(
    `[sandbox-${index}] Dispatching: deepaudit ${config.command} (${manifest.length || "all"} files)...`,
  );
...
export async function launch(config: SandboxConfig, onLog: (msg: string) => void): Promise<string> {
  ...
  onLog(
    `  Collect results later: pnpm deepaudit sandbox collect --project-id ${config.projectId} --run-id ${runId}`,
  );
  onLog(
    `  Check status:          pnpm deepaudit sandbox status --project-id ${config.projectId} --run-id ${runId}`,
  );
...
export async function collect(
  ...
  // the user can `deepaudit sandbox-collect <runId>` again later.
...
async function runOnSandboxAttached(
  ...
  try {
    ...
    const invocation = buildSandboxInvocation(config, mode, manifestPath);
    onLog(
      `[sandbox-${index}] Running: deepaudit ${config.command} (${manifest.length || "all"} files)...`,
    );
