...
const DEEPAUDIT_DIR = "/vercel/sandbox/deepaudit-app";
const DATA_DIR = "/vercel/sandbox/deepaudit-app/data";
const TARGET_DIR = "/vercel/sandbox/target";

export { DATA_DIR, DEEPAUDIT_DIR, TARGET_DIR };

/**
 * Whether the running CLI lives inside the source repo (`dev`) or inside a
 * user's `.deepaudit/node_modules/deepaudit` install (`installed`). Drives:
 *   - which directory gets tarballed and uploaded
 *   - whether `pnpm install --frozen-lockfile` is safe (only in dev, where
 *     we ship our own lockfile; the user's `.deepaudit/` may carry a lockfile
 ...
 */
export type DeepauditMode = "dev" | "installed";
...
export interface UploadBundle {
  /** tar.gz of the local deepaudit app (source only, no node_modules/.git/data) */
  app: TarballStats;
 ...
 */
const BROKERED_TOKEN_PLACEHOLDER = "deepaudit-sandbox-brokered-credential";
...
// Path differs by upload mode (see DeepauditMode):
//   dev       — uploaded source workspace, proxy lives at its source location
//   installed — user's `.deepaudit/` workspace; after `pnpm install` the deepaudit
//               package is materialized under node_modules/deepaudit/, with the
//               proxy script bundled into dist/ by build.mjs.
const PROXY_SCRIPT_BY_MODE: Record<DeepauditMode, string> = {
  dev: `${DEEPAUDIT_DIR}/packages/deepaudit/src/sandbox/request-proxy.mjs`,
  installed: `${DEEPAUDIT_DIR}/node_modules/deepaudit/dist/sandbox/request-proxy.mjs`,
};
...
export function buildSandboxEnv(
  ...
  env["DEEPAUDIT_INSIDE_SANDBOX"] = "1";
...
interface BootstrapOptions {
  ...
  vcpus: number;
  timeout: number;
  /** Source repo vs. user's `.deepaudit/` install — see DeepauditMode docstring */
  mode: DeepauditMode;
  bundle: UploadBundle;
...
export async function createBootstrapSnapshot(opts: BootstrapOptions): Promise<string> {
  ...
  try {
    ...
    const appTar = "/tmp/deepaudit-app.tar.gz";
    const targetTar = "/tmp/deepaudit-target.tar.gz";
    const dataTar = "/tmp/deepaudit-data.tar.gz";
    ...
    await Promise.all([
      (async () => {
        ...
        await extractTarballOnSandbox(sandbox, appTar, DEEPAUDIT_DIR, opts.onLog);
      })(),
      (async () => {
    ...
    // install in the sandbox above. The user's `.deepaudit/` install in
    ...
    await runAndLog(sandbox, "pnpm", installArgs, DEEPAUDIT_DIR, opts.onLog);
...
for ARCH in x64 arm64; do
  ...
  for DEST_VARIANT in "linux-\${ARCH}-musl" "linux-\${ARCH}"; do
    ...
    TARGET_DIR="${DEEPAUDIT_DIR}/node_modules/.pnpm/\${PNPM_KEY}/node_modules/\${DEST_PKG}"
    mkdir -p "\${TARGET_DIR}"
...
DEST_PATHS=(
  "${DEEPAUDIT_DIR}/node_modules/.pnpm/\${PNPM_KEY}/node_modules/\${PLATFORM_PKG}"
  "${DEEPAUDIT_DIR}/node_modules/\${PLATFORM_PKG}"
)
...
async function writeCodexConfig(sandbox: Sandbox, onLog: (msg: string) => void): Promise<void> {
  const configToml = `# Written by deepaudit sandbox bootstrap. Disables non-AI egress
