...
const SETUP_MARKER = "/tmp/deepaudit-setup-done";
...
export async function downloadResults(
  ...
  const remoteTarPath = `/tmp/deepaudit-results-${sandboxIndex}.tar.gz`;
  ...
  const localTarPath = `/tmp/deepaudit-results-${sandboxIndex}-${Date.now()}.tar.gz`;
