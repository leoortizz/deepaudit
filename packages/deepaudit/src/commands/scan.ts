 ...
 *   2. `findProject(id).root` from the loaded `deepaudit.config.ts`
...
function resolveScanRoot(opts: { projectId: string; root?: string }): string {
  ...
  if (projectFromConfig && configPath) {
    ...
    return requireExistingDir(resolved, `deepaudit.config.ts (${configPath})`);
  }
  ...
  throw new Error(
    `No root path for project "${opts.projectId}".\n` +
      `  Pass --root <path>, or add the project to deepaudit.config.ts:\n` +
...
export async function scanCommand(opts: { projectId?: string; root?: string; matchers?: string }) {
  ...
  for (const stat of lowCoverage) {
    ...
    console.log(
      `  ${DIM}deepaudit may not have built-in matchers for your ${stat.language} stack.${RESET}`,
    );
    console.log(
  ...
  console.log(`  ${CYAN}pnpm deepaudit process --project-id ${projectId}${RESET}`);
