...
export function initCommand(opts: InitOpts) {
  // Defaults: scaffold `.deepaudit/` inside the current codebase, with the
  // codebase itself as the first project. Override either by passing
  // explicit positional args.
  const workspaceArg = opts.workspace ?? ".deepaudit";
  ...
  // Sever .deepaudit/ from any ancestor monorepo. npm and yarn walk up looking
  ...
  // see packageJson()) makes `.deepaudit/` its own root regardless of what
  // the parent repo defines. Lets `npm/yarn/pnpm install` from `.deepaudit/`
  // manage only this directory's deps.
  writeFile(workspaceDir, "pnpm-workspace.yaml", pnpmWorkspaceYaml());
  writeFile(workspaceDir, "deepaudit.config.ts", emptyConfigTs());
  ...
  // `init-project` would do: data/<id>/{project.json,RULES.md,SETUP.md}
  ...
  console.log(`  pnpm install                          ${DIM}# installs deepaudit${RESET}`);
  console.log(
  ...
  console.log(`    pnpm deepaudit scan`);
  console.log(`    pnpm deepaudit process`);
  console.log();
  console.log(`  ${DIM}# --project-id is auto-resolved while there's only one project.${RESET}`);
  console.log(`  ${DIM}# Register another codebase later: deepaudit init-project <root>${RESET}`);
}
...
function printAgentPrompt(id: string, targetRel: string): void {
  const lines = [
    ...
    `any existing CLAUDE.md / AGENTS.md / .cursor/rules, plus the`,
    `README and CONTRIBUTING.md, then populate each section of`,
    `data/${id}/RULES.md with the project's actual conventions.`,
    ``,
    `Keep it SHORT — target 50–100 lines total. Each entry should be`,
    `an imperative ("do X", "don't Y", "prefer X"). Skip descriptive`,
    `prose (repo layout, dev commands) — that isn't a rule. Cover only`,
    `what's project-specific; the default pack handles the basics.`,
    `RULES.md is injected into every audit batch; verbose context`,
    `dilutes signal.`,
  ];
...
/**
 * npm rejects package names starting with a dot. `.deepaudit` (the default
...
function workspacePackageName(workspaceDir: string): string {
  ...
  return base.startsWith(".") ? "deepaudit-workspace" : base;
}

function packageJson(name: string): string {
  // Pin the scaffolded dep to the version of deepaudit running this
  ...
  // a user typing `npx deepaudit@latest init` expects.
  const deepauditVersion = `^${getDeepauditVersion()}`;
  return `${JSON.stringify(
    {
      name,
      version: "0.1.0",
      private: true,
      description: "deepaudit scanning workspace",
      type: "module",
      ...
      // parent monorepo's workspace would absorb `.deepaudit/`.
      workspaces: [],
      ...
      // `.deepaudit/` refuse to run. Setting it here stops the walk at the
      // workspace root.
      packageManager: detectPackageManager(),
      dependencies: { deepaudit: deepauditVersion },
    },
    null,
    2,
  )}\n`;
...
function pnpmWorkspaceYaml(): string {
  ...
  // ".deepaudit/ is its own workspace root with no members beyond itself."
  return `packages: []\n`;
}
...
2. Add an AI Gateway / Anthropic / OpenAI token to \`.env.local\`. If
   ...
   \`revalidate\` / \`triage\`); deepaudit auto-detects and reuses the
   subscription. See
...
data/${id}/
  RULES.md                Project rules — checked into git, hand-curated
...
Or browse on
[GitHub](https://github.com/vercel/deepaudit/tree/main/docs).
`;
}

/**
 * Workspace-level AGENTS.md — generic pointer to per-project SETUP.md
 * files and to the deepaudit skill. Per-project setup prompts now live at
...
function gitignore(): string {
  // Keep curated files (RULES.md, SETUP.md) tracked so teammates inherit
  // project context. Ignore the regenerable / heavy / sensitive bits.
  return `node_modules/
.env*.local

# Scan output — regenerated by \`deepaudit scan\` / \`process\`. RULES.md
