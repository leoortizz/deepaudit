...
/**
 * `sandbox <deepaudit-command>` — run attached (default) or detached (--detach)
 */
export async function sandboxCommand(subcommand: string, opts: SandboxOpts) {
  ...
  if (opts.detach) {
    ...
    console.log(
      `  Status:  ${DIM}pnpm deepaudit sandbox status --project-id ${config.projectId} --run-id ${runId}${RESET}`,
    );
    console.log(
      `  Collect: ${DIM}pnpm deepaudit sandbox collect --project-id ${config.projectId} --run-id ${runId}${RESET}`,
    );
    return;
  }
