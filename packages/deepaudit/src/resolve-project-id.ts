...
export function resolveProjectId(provided: string | undefined): string {
  ...
  if (projects.length === 0) {
    throw new Error(
      `No --project-id specified and no projects found in deepaudit.config.ts.\n` +
        `  Run \`deepaudit init\` to scaffold a workspace, or add an entry to\n` +
        `  the projects[] array in your existing deepaudit.config.ts.`,
    );
  }

  const ids = projects.map((p) => p.id).join(", ");
  throw new Error(
    `Multiple projects in deepaudit.config.ts: ${ids}.\n` + `  Pass --project-id <id> to pick one.`,
  );
}
 ...
 * in a freshly-checked-out repo with no prior deepaudit setup.
 *
 * Resolution rules:
 *   1. If `--project-id` is given:
 *      a. Look it up in deepaudit.config.ts → use its declared root.
...
export function resolveProjectIdForDirect(
  ...
  const sanitized = baseName.replace(/[^A-Za-z0-9._-]/g, "-").slice(0, 64) || "deepaudit-target";
  // basename may start with a dot (".deepaudit") which validateProjectId
  // rejects. Strip leading non-alphanumerics.
  const safe = sanitized.replace(/^[^A-Za-z0-9]+/, "") || "deepaudit-target";
