...
// failures.
const SETUP_DOC_URL = "https://github.com/vercel-labs/deepaudit/blob/main/docs/vercel-setup.md";
...
export function renderQuotaMessage(args: {
  ...
  if (onGateway || source === "gateway-credits") {
    ...
    lines.push(
      `  ${YELLOW}deepaudit but generally do not have enough headroom for full repo scans.${RESET}`,
    );
    lines.push("");
  ...
  lines.push("");
  lines.push(
    `  ${DIM}After fixing, re-run:  deepaudit ${command} --project-id ${projectId}${RESET}`,
  );
  lines.push("");
