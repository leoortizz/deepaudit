...
describe("assertAgentCredential", () => {
  ...
  beforeEach(() => {
    ...
    emptyClaudeHome = mkdtempSync(join(tmpdir(), "deepaudit-claude-home-"));
    emptyCodexHome = mkdtempSync(join(tmpdir(), "deepaudit-codex-home-"));
    emptyPathDir = mkdtempSync(join(tmpdir(), "deepaudit-empty-path-"));
  ...
  it("throws actionable message for claude-agent-sdk when no token and no claude CLI", () => {
    ...
    expect(() => assertAgentCredential("claude-agent-sdk")).toThrow(
      /https:\/\/github\.com\/vercel-labs\/deepaudit\/blob\/main\/docs\/vercel-setup\.md/,
    );
  });
