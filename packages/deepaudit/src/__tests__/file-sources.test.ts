...
function tempRepo(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "deepaudit-fs-"));
...
describe("resolveFiles()", () => {
  ...
  it("filters generated deepaudit data records from explicit file lists", () => {
    const root = tempRepo();
    const oldDataRoot = process.env.DEEPAUDIT_DATA_ROOT;
    process.env.DEEPAUDIT_DATA_ROOT = path.join(root, "data{prod,dev}");
    cleanups.push(() => {
