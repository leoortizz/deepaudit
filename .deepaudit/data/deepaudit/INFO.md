...
deepaudit is a developer CLI — a TypeScript pnpm monorepo shipping the `deepaudit`
binary plus the `deepaudit/config` sub-export. A developer runs `deepaudit scan` /
`process` from a `.deepaudit/` workspace inside their own repo to find security
...
files and writes JSON under `.deepaudit/data/<id>/`.

## Trust model — read this first; it controls almost every triage call

**deepaudit runs as the developer, on the developer's inputs.** The vast majority
...
- **`deepaudit.config.ts`** loaded via `jiti` in
  `packages/deepaudit/src/load-config.ts` — the developer's own code. Loading
...
- **`execSync` / `spawn` calls in `packages/core/src/run.ts`,
  `packages/processor/src/agents/`, and `packages/deepaudit/src/`** when their
...
the host ingests from it (`packages/deepaudit/src/sandbox/{download,upload}.ts`,
...
- **Provider tokens** (`ANTHROPIC_AUTH_TOKEN`, `OPENAI_API_KEY`,
  ...
  CLI startup.
- **Vercel Sandbox auth** in `packages/deepaudit/src/sandbox/` — OIDC tokens
  (local) and access tokens (CI), passed to `@vercel/sandbox`.
- **Plugin contract** — `DeepauditPlugin` modules from `deepaudit.config.ts` are
  fully trusted (developer's own code). Not a boundary.

## Things worth flagging (real signals, not noise)

- **Sandbox output handling** in `packages/deepaudit/src/sandbox/` —
...
- **CI workflow auth scoping** — `.github/workflows/deepaudit.yml` deliberately
